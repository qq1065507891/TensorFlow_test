...
def read_and_decode(filename):
    ...
    label1 = tf.cast(features['label0', tf.uint8])
    label2 = tf.cast(features['label0', tf.uint8])
    label3 = tf.cast(features['label0', tf.uint8])
...
with tf.Session() as sess:
    ...
    one_hot_labes0 = tf.one_hot(indices=tf.cast(y0, tf.uint8), depth=CHAR_SET_LEN)
    one_hot_labes1 = tf.one_hot(indices=tf.cast(y1, tf.uint8), depth=CHAR_SET_LEN)
    one_hot_labes2 = tf.one_hot(indices=tf.cast(y2, tf.uint8), depth=CHAR_SET_LEN)
    one_hot_labes3 = tf.one_hot(indices=tf.cast(y3, tf.uint8), depth=CHAR_SET_LEN)
